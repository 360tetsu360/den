...
pub fn derive_den(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let decode = fields.named.iter().map(|field| {
        ...
        match attr_each {
            ...
            Some(AttributeType::DenWith(with)) => {
                ...
                return quote! {
                    #ident : <#den_with as DenWith<#ty>>::decode(bytes)?
                };
            }
            None => {
                return quote! {
                    #ident : Den::decode(bytes)?
                }
            }
        }
    });
    ...
    let encode = fields.named.iter().map(|field| {
        ...
        match attr_each {
            ...
            Some(AttributeType::DenWith(with)) => {
                let den_with = Ident::new(&with.value(), with.span());
                return quote! {
                    <#den_with as DenWith<#ty>>::encode(&self.#ident, bytes)
                };
            }
            None => {
                return quote! {
                    Den::encode(&self.#ident,bytes)
                }
            }
        }
    });
    ...
    let len = fields.named.iter().map(|field| {
        ...
        match attr_each {
            ...
            Some(AttributeType::DenWith(with)) => {
                let den_with = Ident::new(&with.value(), with.span());
                return quote! {
                    + <#den_with as DenWith<#ty>>::size(&self.#ident)
                };
            }
            None => {
                return quote! {
    ...
    let gen = quote! {
        impl Den for #struct_name {
            fn decode(bytes : &mut std::io::Cursor<&[u8]>) -> std::io::Result<Self> {
                Ok(Self {
                    #(#decode),*
                })
            }
            fn encode(&self,bytes : &mut bytes::BytesMut) {
                #(#encode);*
            }
            fn size(&self) -> usize {
                let size = 0
